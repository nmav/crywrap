...
static gnutls_dh_params_t dh_params; /**< GNUTLS DH parameters. */
static gnutls_datum_t dh_file = { (void *) _crywrap_prime_dh_1024, sizeof(_crywrap_prime_dh_1024) };
 ...
 * Used by the argp suite.
 */
const char *argp_program_version = __CRYWRAP__ " " PACKAGE_VERSION;
...
_crywrap_do_one(const crywrap_config_t * config, int insock, int outsock)
{
	...
	gnutls_transport_set_int2(session, insock, outsock);
